...
! and indexing. Elements of integer arrays to be tabulated are still
! kind=our_int
!#######################################################################
module tabulate
   ...
   type dnode
      ...
      integer(kind=long_int) :: frequency = 0
   ...
   type :: table_type
      ...
      integer(kind=long_int) :: length=0
      integer(kind=long_int), pointer :: freq(:)=>null()
      ...
      integer(kind=long_int) :: write_posn=0
   end type table_type
   !##################################################################
contains
   !################################################################
   integer(kind=our_int) function nullify_table(table, err) &
      ...
      table%length = 0
      table%write_posn = 0
      if( associated(table%freq) ) deallocate(table%freq, stat=status)
...
800   call err_handle(err, 2, whichsub = subname, whichmod = modname )
   ...
   integer(kind=our_int) function tabulate_real_variable(vec, &
      ...
      ! tabulates a vector of double precision reals
      implicit none
      ! args
...
700   continue
      ...
      call err_handle(err, 3, iobs = int(i,kind=our_int) )
...
800   call err_handle(err, 2, whichsub = subname, whichmod = modname )
   ...
   integer(kind=our_int) function tabulate_integer_variable(vec, &
      ...
      ! locals
      integer(kind=long_int) :: i
      integer(kind=our_int) :: number, ijunk
...
800   call err_handle(err, 2, whichsub = subname, whichmod = modname )
   ...
   integer(kind=our_int) function tabulate_string_variable(vec, &
      ...
      ! begin
      answer = RETURN_FAIL
      if( size( vec, kind=long_int ) == 0 ) goto 600
...
999   continue
   ...
   integer(our_int) function write_real_tree_to_table(t, table, err) &
      ...
      if( associated(table%freq) ) deallocate(table%freq, stat=status)
      if( status /= 0 ) goto 700
      allocate( table%freq(table%length), stat=status )
...
800   continue
   ...
   integer(our_int) function get_table_length(length, table, err) &
      ...
      implicit none
      ! args
      integer(long_int), intent(out) :: length
...
800   call err_handle(err, 2, whichsub = subname, whichmod = modname )
   ...
   integer(our_int) function get_table_frequencies(freq, table, err) &
      ...
      implicit none
      ! args
      integer(kind=long_int), pointer :: freq(:)
      type(table_type), intent(in) :: table
      type(error_type), intent(inout) :: err
      integer(kind=our_int) :: status
      ! locals
      ...
      status = 0
      if( table%is_null ) goto 700
      if( associated(freq) ) deallocate(freq, stat=status)
      if( status /= 0 ) goto 720
      allocate( freq(table%length), stat=status )
      if( status /= 0 ) goto 730
      freq(:) = table%freq(:)
      ! normal exit
      answer = RETURN_SUCCESS
      return
